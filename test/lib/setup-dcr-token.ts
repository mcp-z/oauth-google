...
/**
 * DCR Token Setup Helper for Google OAuth
 *
 ...
 */
async function _refreshGoogleAccessToken(clientId: string, clientSecret: string | undefined, refreshToken: string): Promise<{ accessToken: string; expiresAt: number }> {
...
export async function setupDcrToken(options: SetupDcrTokenOptions): Promise<void> {
  ...
  try {
    ...
    const registration = await registrar.registerClient(capabilities.registrationEndpoint, {
      clientName: 'DCR Test Setup (Google)',
    ...
    await dcrStore.set('google', dcrTokenData);
