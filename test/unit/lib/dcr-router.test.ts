...
describe('unit/dcr-router-scope-fallback', () => {
  it('uses server scopesSupported when client omits scope parameter', async () => {
    ...
    const serverScopes = ['openid', 'https://www.googleapis.com/auth/spreadsheets'];
    ...
    try {
      ...
      assert.strictEqual(response.status, 302, 'Should redirect to Google OAuth');

      const location = response.headers.get('location');
      assert.ok(location, 'Should have Location header');

      const googleUrl = new URL(location);
      const scopeParam = googleUrl.searchParams.get('scope');

      // The scope should be the server's default scopes, not empty
      assert.ok(scopeParam, 'Google OAuth URL should have scope parameter');
    ...
    try {
      ...
      globalThis.fetch = (async (input: Parameters<typeof fetch>[0], init?: Parameters<typeof fetch>[1]): Promise<Response> => {
        ...
        if (url === 'https://oauth2.googleapis.com/token') {
          return new Response(JSON.stringify({ access_token: 'google-access-token', refresh_token: 'google-refresh-token', expires_in: 3600, scope: 'openid' }), {
            status: 200,
            headers: { 'Content-Type': 'application/json' },
          });
        }
      ...
      try {
        ...
        await store.set('dcr:google-state:dcr-iss-test-state', {
          client_id: clientId,
          redirect_uri: redirectUri,
          scope: 'openid',
          state: clientState,
        ...
        const response = await fetch(`${baseUrl}/oauth/callback?code=google-auth-code&state=dcr-iss-test-state`, { redirect: 'manual' });
